    ...
    <>
      ...
      >
        <nav
          ...
          aria-label="Primary"
          style={{ height: scrolled && !open ? "4rem" : "var(--header-h)" }}
        >
          <Link
                ...
                >
                  <span
                    className={cn(
                      ...
                      isActive(l.href) && "border-gold-500",
                    )}
                  >
                    {l.label}
                  </span>
          ...
          <div className="hidden items-center gap-2 xl:flex">
            <LanguageSwitcher
              ...
              ariaLabel={labels.switchLocaleLabel}
              tone="dark"
            />
            <Button
              asChild
            ...
            >
              <a
                ...
                onClick={() =>
                  track({ event: "whatsapp_click", location: "header" })
                }
              >
                <MessageCircle />
              </a>
      ...
      {/*
        Rendered as a SIBLING of <header>, never inside it. When the menu is
        open the header has a backdrop-filter, which makes it the containing
        block for any fixed-position descendant: the panel's top/bottom would
        then be measured against the 73px header and collapse to height 0.
        As a sibling its containing block is the viewport. z-[45] sits above
        the z-40 mobile sticky bar (the panel has its own Call/WhatsApp) and
        below the z-50 header so the close button stays on top.
      */}
      {open && (
        <div
          ref={panelRef}
          ...
          className="fixed inset-0 z-[45] overflow-y-auto bg-primary-900 pt-[var(--header-h)] xl:hidden"
        >
              ...
              <div className="grid grid-cols-2 gap-3">
                <Button asChild variant="outlineLight" size="lg">
                  <a
                    href={phoneHref}
                    onClick={() =>
                      track({ event: "call_click", location: "mobile_menu" })
                    }
                  >
                    <Phone /> {labels.call}
                  </a>
